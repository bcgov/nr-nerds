...
async function getCurrentSprintOptionId() {
  ...
  // Find the option with a date range that includes today
  const today = new Date();
  for (const opt of sprintField.options) {
    // Accept formats like 'Sprint 1 (2025-05-10 to 2025-05-24)' or 'Sprint 1 (2025-05-10 – 2025-05-24)'
  ...
  // If no match, log all options for debugging
  console.warn(`No Sprint option with a date range including today (${today.toISOString().slice(0,10)}). Available options: [${sprintField.options.map(o => `'${o.name}'`).join(', ')}]`);
  return null;
}
